...
class SenselDevice():
    ...
    def readReg(self, reg, size):
        ...
        if not self._verifyChecksum(resp, resp_checksum):
            logging.error(
                "Response checksum didn't match checksum (resp_checksum=%d, checksum=%d)" % (resp_checksum, checksum))
            raise SenselSerialReadError(1, 1)
...
class SenselRegisterWriteError(SenselError):
    ...
    def __init__(self, reg, size, data, ack_received, response):
        ...
        logging.error("Failed to write register 0x%02X (size=%d, ack_received=%d, response=%d)" % (
        reg, size, ack_received, response))